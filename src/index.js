...
// Import radio stations from separate config file AS REQUESTED
    ...
    for (const url of urlsToTry) {
      try {
        ...
        if (result.loadType === 'track' && result.data) {
          ...
          await player.playTrack({
            track: { encoded: result.data.tracks[0].encoded }
          });
          await new Promise(resolve => setTimeout(resolve, 2000));
          console.log(`✅ Successfully started ${station.name} with ${url}`);
          return { success: true, station, url };
...
console.log('🎬 One Piece radio bot initialization started');import 'dotenv/config';
import http from 'node:http';
