    ...
    async function initializePersistentRadio() {
      try {
        ...
        if (fs.existsSync(bannerPath)) {
          ...
          files.push(bannerAttachment);
          console.log('✅ Banner GIF attachment created');
        } else {
          console.warn('⚠️ Banner GIF file not found at:', bannerPath);
...
console.log('🎤 Uta DJ Bot initialization started');import 'dotenv/config';
import http from 'node:http';
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
